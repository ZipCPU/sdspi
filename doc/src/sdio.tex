...
\includegraphics[height=2.5in]{sdioblocks.eps}
...
\begin{figure}\begin{center}
\includegraphics[height=2.5in]{sdcmdflow.eps}
...
\begin{figure}\begin{center}
\includegraphics[height=2.5in]{sdiotxflow.eps}
...
\begin{figure}\begin{center}
\includegraphics[height=2.5in]{mdlsdio.eps}
...
Fig.~\ref{fig:vlogtb}.
\begin{figure}\begin{center}
\includegraphics[height=2.5in]{vlogtb.eps}
...
\begin{figure}\begin{center}
\includegraphics[height=2.5in]{swlayers.eps}
...
reflected in the ``Card busy'' bit.  Although there is a command type for
expecting an ``R1b'' command, this command type is not (currently) connected
to this register.  Once the card raises DAT[0], the ``Card busy'' line will
be cleared automatically and will not reassert until the next command completes
at the earliest.
...
1'b0 & CRC error.  Set if a sector is received with an invalid CRC.  May also
	be an indication that the received sector had started to arrive before
	the command to receive that sector had been given.\\
1'b1 & Watchdog timeout.  Set if no bits are received within the watchdog
	timeout period.\\
%% }}}
...
W & 2 & Width.  Controls how many data bits are used in any data transaction.
	...
	DAT[7:0].  \\
D$_S$&1& Data strobe.  Set to one to use the card's data strobe, clear
	otherwise.  Parameters {\tt OPT\_EMMC} and {\tt OPT\_DS} must also
	be set to use this capability.\\
...
%% 8'h41 ->  400kHz
%% 8'h1b -> 1MHz
N & Sets clock frequency to $50 / \left(N-1\right)$~MHz.
%% }}}
