...
2.0pre & 7/08/2026 & Gisselquist & eMMC Boot support, and trim registers\\\hline
\end{revisionhistory}
% Revision History
...
such as HS400.  If this happens, the controller will likely miss the start
bit of that second block and (assuming it's not all ones) then fail its CRC.
To prevent this from happening, the clock must stop long enough between blocks
to allow the controller to reset itself prior to receiving the next block.
This control helps to make sure the clock stops early enough at higher speeds.
...
incoming data in the center of the eye.

Turning to the RX TRIM register, with zero RX TRIM, the receive clock will
stop once the last incoming clock's worth of data is registered.  This
includes both data and CRC.  Hence, in 1b mode, a 512 Byte sector will require
4112~clocks containing return data, whereas in 8b mode the same sector would
require 528~clocks.  Once this number, less the RX TRIM, has been received,
a flag will be set to allow the clock to stop.  From that point on, no more
clocks will be generated for this receive block.
