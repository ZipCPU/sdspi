...
2.0 & 8/01/2026 & Gisselquist & eMMC Boot support, and trim registers\\\hline
\end{revisionhistory}
% Revision History
...
this new IP took on a different set of goals.  First and foremost, it was to
...
write commands not supported by the SDSPI driver.  This controller was also
built to support eMMC devices--to include both the data strobe and the eMMC
device interrupt.  Unlike the SDSPI controller, low logic was no longer one
of my goals.
% It does not include any
% of the low power UHS-II modes used by the newest cards of today, nor any of
% the SDPCIe modes--those are different protocols.
\end{preface}
...
\item 32-bit Wishbone control interface.

	An optional AXI-Lite control interface also exists.
...
\item An optional DMA interface exists.  This DMA interface can be used to
	...
	The DMA interface can master either Wishbone or AXI4 buses, and it is
	not limited to 32b bus widths.
\end{itemize}
...
\item While the IP has been designed to support all commands, the verification
	model doesn't (yet) support all SDIO commands.

	As additional commands are required, they will be added.  At present,
	however, the SDIO and eMMC models are both sufficient to verify IP
	startup and basic data transfer.  In addition, the eMMC model is also
	sufficient to verify the eMMC BOOT capability.
\end{itemize}
%% }}}
...
all digital logic component whose top level is
\zhref{../rtl/sdio/sdio.v}{\tt sdio.v},
and a hardware dependent front end component whose top level is found in
\zhref{../rtl/sdio/sdfrontend.v}{\tt sdfrontend.v}.
These two are combined together in a top level SDIO component,
\zhref{../rtl/sdio/sdio\_top.v}{\tt sdio\_top.v}.

The core controller, found in \zhref{../rtl/sdio/sdio.v}{\tt sdio.v}, itself is
composed of five separate components.  The first,
\zhref{../rtl/sdio/sdwb.v}{\tt sdwb.v}, is the controller.  This component is
...
this controller before going to the card, or after coming from the card.

A second, alternative, controller implementation exists in
\zhref{../rtl/sdio/sdaxil.v}{\tt sdaxil.v}.  This controller is nearly line
for line identical to the \zhref{../rtl/sdio/sdwb.v}{\tt sdwb.v} controller,
save that interaction with this controller takes place over AXI-Lite instead
of Wishbone.

The controller maintains a PHY register for configuring the front end.  This
register also controls the clock generator, \zhref{../rtl/sdio/sdckgen.v}{\tt sdckgen.v}.  This clock
...
Commands issued to the controller will be forwarded to the \zhref{../rtl/sdio/sdcmd.v}{\tt sdcmd.v}
...
either 48-bit or 136-bit command responses.  Responses are checked for
potential CRC or frame errors.  If the device fails to return a response
...
For commands, a data register within \zhref{../rtl/sdio/sdwb.v}{\tt sdwb.v} is first set, then the
command is issued.  The \zhref{../rtl/sdio/sdcmd.v}{\tt sdcmd.v} module organizes both into a 48-bit
word to be transmitted.  The \zhref{../rtl/sdio/sdcmd.v}{\tt sdcmd.v} module then looks for one of
...
placed into one of the two FIFOs in \zhref{../rtl/sdio/sdwb.v}{\tt sdwb.v}.
...
The \zhref{../rtl/sdio/sdrxframe.v}{\tt sdrxframe.v} component is activated when data is expected from the
...
one of the FIFOs in \zhref{../rtl/sdio/sdwb.v}{\tt sdwb.v}.  If a full frame, as defined by the
...
Similarly, the \zhref{../rtl/sdio/sdtxframe.v}{\tt sdtxframe.v} command is activated when data is to be
sent over the data wires to the device.  In this case, data flows from one
of the FIFOs in \zhref{../rtl/sdio/sdwb.v}{\tt sdwb.v} to \zhref{../rtl/sdtxframe.v}{\tt sdtxframe.v}, gets formatted for the
...
\end{center}\end{figure}
The \zhref{../rtl/sdio/sdtxframe.v}{\tt sdtxframe.v} transmit component can
also be configured to expect and wait for a CRC token following a successful
transfer.
...
%% {{{
The DMA is an add-on to the basic architecture, and as a result is an optional
feature of the design.  It is built from several parts.  The first key
component to the DMA is found in the control module,
\zhref{../rtl/sdio/sdwb.v}{\tt sdwb.v}.  When the DMA is present, the control
module becomes responsible for sectioning transfers into per-block requests,
and forwarding data to and from the DMA module itself.  If buffers fill while
reading, it will automatically stop the clock.  Once all data has been read,
the control module will issue a CMD12 ({\tt STOP\_TRANSMISSION}) to end the
transfer.\footnote{When using the DMA to perform an alternative eMMC boot
sequence, the transfer will end with a CMD0 ({\tt GO\_IDLE}).}  The rest of
the DMA logic is found in the DMA module,
\zhref{../rtl/sdio/sddma.v}{\tt sddma.v}.  The structure of this module is
shown in Fig.~\ref{fig:dmablocks}.
\begin{figure}\begin{center}
...
\zhref{../rtl/sdio/sdax\_mm2s.v}{\tt sdax\_mm2s.v}) component.  Finally, when reading
...
A small amount of logic exists in the \zhref{../rtl/sdio/sddma.v}{\tt sddma.v}
module to control the switching between sources and outputs, as well as to
make sure block boundaries are respected.
%% }}}
\section{eMMC BOOT Architecture}\label{sec:arch-boot}
%% {{{
A second optional addition to the controller architecture is the eMMC BOOT
capability.  This additional capability is primarily handled by an upagrde
to the control module, whether 
\zhref{../rtl/sdio/sdwb.v}{\tt sdwb.v} or
\zhref{../rtl/sdio/sdaxil.v}{\tt sdaxil.v}.  This additional logic is enabled
by setting both {\tt OPT\_EMMC} and {\tt OPT\_BOOTEN}.

This logic can be used to hold the command wire low (CMD), wait for a device
acknowledgment, and then transfer block after block of data to memory--either
via the DMA or (with CPU/User interaction) from the FIFOs.
%% }}}
...
(FSM) at the top level, in either
\zhref{../bench/verilog/mdl\_sdio.v}{\tt mdl\_sdio.v} or
\zhref{../bench/verilog/mdl\_emmc.v}{\tt mdl\_emmc.v}.
...
This test bench also supports the SDSLAVE component, discussed elsewhere.
This component is a slave to the SDIO bus.  It is a separate product,
though also found and released in this repository.  If present, this component
will replace the SDIO model, 
\zhref{../bench/verilog/mdl\_sdio.v}{\tt mdl\_sdio.v}.  By placing it in the
top level test bench, the SDSLAVE component may be tested as a slave to the
SDIO controller, if desired, using many of the same test scripts as are used
to test the SDIO controller itself..

Two versions of this top-level interface exist.  The first supports all
Wishbone interactions.  It can be found in
\zhref{../bench/verilog/tb\_wb.v}{\tt tb\_wb.v}.  A second top level exists for
testing AXI interactions.  This top level test bench may be found in
...
\end{zCpp}
Only after clearing the {\tt SDIO\_REMOVED} flag may additional work take
place.

If {\tt SDIO\_REMOVED} bit is ever set from here on out, it will indicate that
the SD card has been removed.  If the command register indicates a card is
present, yet the removed flag is still high, it may be that a card was removed
and a new one inserted.  Either way, the controller will need to restart
the card with a {\tt GO\_IDLE} command.

%% }}}
\section{Issuing a Hardware Reset}
%% {{{
Unlike the SDIO interface, the eMMC interface often includes a hardware reset
pin.  This pin is controlled from the same control register.  To enable the
reset, one need only write to the command register with the reset bit present.
\begin{zCpp}
	const	unsigned	EMMC_HWRESET  = 0x02000000;
	unsigned	cmd;

	// Enable the reset
	_sdio->sd_cmd = EMMC_HWRESET;
	...
	do {
		cmd = _sdio->sd_cmd;
	} while(cmd & EMMC_HWRESET);
\end{zCpp}
Writing a NULL command, {\tt SDIO\_NULLCMD}, can be used to clear the reset
indication.

Beware when using this reset that eMMC has a minimum reset length requirement.
Hence, just because the reset has been commanded to be cleared doesn't mean
that it has been cleared yet.  For this reason, the reset bit of the command
register may then be polled to know if and when the device has completed its
reset.

While SD cards don't necessarily have a reset pin, there are reasons to control
the power to an SD Card.  One such reason might be to enable the card to
transition back from 1.8~V to 3.3~V modes.  The hardware reset pin used in
eMMC may also be used to control a power pin if so desired.
%% }}}
\section{Querying PHY Capability}\label{sec:phy-query}
%% {{{
It can be important to know how the SDIO controller was configured, so let's
continue there next.  Specifically, let's start by trying to determine which
version of the PHY is present.  We can do this by requesting a 200~MHz clock,
an 8~bit bus width, DDR mode, and a full PHY shift.  We'll also allow the
device to turn the clock off when not in use, so we that we don't push a high
...
\item The \cppinline{SDPHY_SHFTMSK} specifies 5 bits of a shift register
	...
	all bits remain set in \cppinline{phy}, then design was built
	with the SERDES front end.  If all but the bottom two bits are
...
\item Finally, the PHY register will indicate the maximum sector size
	available to it--typically 512~Bytes, but it might be larger.
	This size is given in bytes by the expression:

	\cppinline{sz = (1 << ((phy>>28)&0x0f));}.
\end{itemize}
...
no stream capability is available, then the return value by itself
...
Note that these tests, and any initial PHY setup, may still be accomplished
even if the eMMC hardware reset wire is active (low).
%% }}}
...
%% }}}
\section{GO IDLE: Commands that don't get responses}\label{sec:go-idle}
%% {{{
...
response, a 48-bit response followed by a busy (R1b), or a 136-bit (R2)
response.
...
sequence, and check it again when you are done to see if and where any error
took place.
...
if the card has ever been subsequently removed and (if so) if you need to start
your initialization sequence again from the {\tt SEND\_GO\_IDLE} command.
...
the clock will only change in between clock periods.  Hence, if the clock was
set for 100~kHz operation, we may need to wait up to 10~$\mu$s for the clock
to switch speeds.  We can tell when the switch is complete by reading back
the lower 8-bits from the PHY register to know the currently set clock speed.

\begin{zCpp}
	// Wait for the clock change speeds
...
Also, by setting the {\tt SDIO\_ACK} bit, the controller will know to wait,
for the acknowledgment token once it has completed its transmission.  Failure
to receive this token, whether because the NAK token was returned or no token
was returned, will result in an error when set.
%% }}}
...
\begin{zCpp}
	// First, wait for the last transmission to complete
	while(_sdio->sd_cmd & SDIO_BUSY)
		;

	// Then send a STOP_TRANSMISSION request
...
In this case, all the ping--pong buffer handling will be handled internally
by the DMA controller.  Likewise, there's no need to send any
{\tt STOP\_TRANSMISSION} command, since the DMA will send it automatically
once all sector data has been transferred to the card.
...
fed to the controller via the AXI stream interface, throttled only by the
...
If this happens, the external stream source may still need to be flushed.
...
In the first case, if the chip returns an R1 response, then the command is
...
\section{eMMC BOOT}
%% {{{
The eMMC standard allows for two separate types of boot commands, a normal one
and an alternate boot command.  If {\tt OPT\_EMMC} and {\tt OPT\_BOOTEN} are
both set, then the design will support both.

The first type of boot command may be entered following a hardware reset.
No software interaction is required, save to wait for the boot to complete.
In this case, the user will want to set the following parameters:

\begin{itemize}
\item Set {\tt OPT\_AUTOBOOT} to enable an automatic boot upon reset
\item Set {\tt BOOT\_TOKEN} if the eMMC has been configured to acknowledge
	the boot request before sending any data (recommended)
\item Set the {\tt BOOT\_ADDR} to the memory address where the BOOT information
	is to be stored by the DMA (assuming {\tt OPT\_DMA} is also
	set).\footnote{While an automatic boot may take place without using
	the DMA, in many ways this defeats the purpose: why load boot
	information in this manner if the CPU already has boot instructions?}
\item Set the number of blocks (512~Byte sectors) to be transferred as part of
	any BOOT operation via {\tt BOOT\_BLOCKS}
\item Set the default clock speed for booting via {\tt BOOT\_SPEED}.
\item Set the protocol used for booting via {\tt BOOT\_MODE}.

	\begin{itemize}
	\item Set {\tt BOOT\_MODE[3]} to use the data strobe, DS, pin to
		control read sampling.  (i.e. HS400)

	\item Set {\tt BOOT\_MODE[2]} to BOOT in DDR mode.

	\item The bottom two bits of the {\tt BOOT\_MODE} control the width,
		using the enumeration shown in Tbl.~\ref{tbl:PHY-WIDTH}.
		Hence, {\tt BOOT\_MODE[1:0]==2'b10} will set an 8b boot width,
		{\tt BOOT\_MODE[1:0]==2'b01} setting a 4b transfer width,
		and {\tt BOOT\_MODE[1:0]==2'b00} setting a 1b transfer width.
		(Note that a 1b transfer width is not the same as SPI mode.)
	\end{itemize}
\end{itemize}

The startup sequence will then start with an external reset, assuming
{\tt OPT\_HWRESET} is set.  Once this reset has been set for a minimum number
of cycles (1uS), the reset will clear and the command line will be held low.
If {\tt BOOT\_TOKEN} is set, the design will wait for the device to acknowledge
the boot request.  Then it will wait for the device to send {\tt BOOT\_BLOCKS}
blocks of data, copying each to RAM as it arrives.  Once complete, an interrupt
will be generated and the automatic boot cycle will be complete.

This cycle can also work without the DMA.  In this case, the user will be
required to unload each block from the FIFO as its data becomes available.

This same sequence may also be commanded by the CPU via the control port.
Since the normal boot mode must be entered following a reset, the design should
first be placed in reset.  This may be done via the external reset pin,
\begin{zCpp}
	_sdio->sd_cmd = EMMC_HWRESET;
\end{zCpp}
or via a special CMD0 ({\tt GO-IDLE}) command.
\begin{zCpp}
	_sdio->sd_data = 0xf0f0f0f0;
	_sdio->sd_cmd  = 40;

	wait_while_busy();
\end{zCpp}
Once the device has been reset, the design parameters may be entered and/or
adjusted as necessary.
\begin{zCpp}
	//	SDIO_ERR clears any prior errors
	//	The R1b response to a null-command invokes the boot
	const	SDIO_BOOT    = 0x04008300;
	...
	// Set the device for the appropriate boot mode
	_sdio->sd_phy = BOOT_PHY_SETUP;	// This would need to be user defined...
\end{zCpp}
If using the DMA, we can also set the DMA (destination) address, and the number
of blocks to transfer.
\begin{zCpp}
	// Set up the DMA for the boot
	_sdio->sd_dma_addr   = mem_destination;
	_sdio->sd_dma_length = block_count;
\end{zCpp}
Finally, we can issue the boot command an wait for the subsequent interrupt.
\begin{zCpp}
	CLEAR_INT(SDIO_INT);	// Clear any current interrupts

	// Command the boot
	//	SDIO_ACK tells the controller to expect a card acknowledgment
	_sdio->sd_cmd = SDIO_BOOT | SDIO_ACK;

	// Now wait for the boot to complete
	WAIT_INT(SDIO_INT);	// Wait for the given interrupt
\end{zCpp}
Note that if the hardware reset bit has been made active, IP will hold the
hardware reset active until its release.  This release command can be given at
the same time as the BOOT command.  Even if this release request comes before
the full eMMC reset has been completed, the hardware will still complete the
reset before starting the BOOT sequence.

We now switch to the alternative boot sequence.  Unlike the normal boot
sequence, the alternate boot sequence is closer to the normal flow of the IP
described elsewhere in this document.  First, the eMMC is reset via the special
{\tt GO-IDLE} command.
\begin{zCpp}
	_sdio->sd_data = 0xf0f0f0f0;
	_sdio->sd_cmd  = 40;
	...
	wait_while_busy();
\end{zCpp}
Before issuing the alternate boot command, it makes sense to setup the DMA.
\begin{zCpp}
	// Set up the DMA for the boot
	_sdio->sd_dma_addr   = mem_destination;
	_sdio->sd_dma_length = block_count;
\end{zCpp}
Now we issue a second {\tt GO-IDLE} command, this time with the argument
required to activate the alternate boot mode.
\begin{zCpp}
	const	SDIO_ALTBOOT    = 0x00008040;	// This is also a GO-IDLE CMD0

	_sdio->sd_data = 0xffffffa;
	_sdio->sd_cmd  = SDIO_ALTBOOT | SDIO_ACK | SDIO_DMA;

	wait_while_busy();
\end{zCpp}
Once the command completes, all that's left is to check the status, handle
any potential errors, and boot the CPU.
%% }}}
...
PHY TRIM&{\tt 0x08} & 32 & R/W & Control sub-clock input delays\\\hline
RX TRIM &{\tt 0x09} & 32 & R/W & Controls RX clock pre-shutdown\\\hline
\end{reglist}
...
%% {{{
\begin{tabular}{|p{1.2in}|p{0.5in}|p{3.5in}|}\hline
	\rowcolor[gray]{0.85} Name & Width & Description\\\hline\hline
P$_B$ &1& {\tt OPT\_BOOTEN}, can be used to tell if the design has been
	built with the BOOT capability enabled.  \\
E$_B$ &1& BOOT Error.  This will be set if any error, such as a failure to
	return a boot acknowledgment or a DMA error, takes place during BOOT
	activity.  \\
A$_B$ &1& BOOT Active.  This bit will be set as long as any BOOT operations
	are active.  \\
K &1& Expect CRC token.  Both eMMC and SDIO protocols return CRC
	acknowledgments (or negative acknowledgments) following the
	transmission of data sectors.  eMMC also returns an acknowledgment
	following a BOOT request.  If set, the controller will look for
	and expect such a token.  If the token is not present within a minimal
	timeout period, an error will be declared.  If not set, no such token
	timeout errors will be generated.  \\
...
C$_t$&1& Data transfer error code.  Following a transfer error, this bit
	will indicate the cause of the error.  If the bit is zero, the design
	either timed out waiting for data or the transfer acknowledgment to
	arrive (assuming $K$ was set).  This bit will be set following either
	a receive CRC error, or a transmit negative acknowledgment token. \\
...
E$_c$&1& Command error.  Set if the E bit is set due to an error in command
	wire processing.  \\
C$_B$&1& Card is busy following an R1b command.  This bit will be set if the
	card asserts a busy signal by lowering DAT[0] following an R1b command.
	It will clear when the busy clears.\\
P & 1 & No card present.  Set if the card detect bit indicates no card is
	currently present.  Automatically clear upon insertion.  This facility
	is only available if the {\tt OPT\_CARD\_DETECT} parameter is set.\\
...
Starting at the top, the first three bits are associated with the BOOT
capability.  P$_B$ will be set if the design has been built with a BOOT
capability within it.\footnote{Note that this capability may be removed
in a future release if it isn't found to be useful.}  E$_B$ will be set
following any BOOT operation error.  If set, the error bit (bit~15) will
also be set.  Finally, A$_B$ will indicate if a boot operation is currently
active and ongoing.

Bit H is the reset bit.  Anytime this bit is set, the reset pin output will
be active (low).  If set, the hardware reset will be set
for a minimum of 100$\mu s$.  This pin must be cleared manually, allowing
the external reset pin to be controlled by the CPU.  While in reset, any
command requests will be ignored.\footnote{The BOOT command is an exception to
this rule.  The BOOT command may be issued while the hardware is still in
reset, as long as the BOOT command also releases the reset signal.}

Because of this delay in clearing, the reset pin should be checked before
going forwards.
...
\item If bits [7:6] are both zero, a new command will be issued if the device
	...
	state.  Also, if bits [9:8] are both set, then a BOOT sequence will
	be initiated.
\end{itemize}
...
1 & ? & 0 & 2'b00 & No command will be sent, but a DMA operation will
	begin. If the reply selected is for an R1b response, this will
	initiate a BOOT sequence with DMA support.\\
0 & 1 & 0 & 2'b00 & No command will be sent, but a memory operation will
	begin.  If the reply selected is for an R1b response, this will
	initiate a BOOT sequence with FIFO instead of DMA support. \\
...
\end{center}\end{table}
Other than any reset requests (hard or soft), {\tt GO\_IDLE} and self--replies
to an eMMC {\tt GO\_IRQ\_STATE} command, all writes to the command register
will be ignored while it is busy.
...
2'b10 & R2 (136b) response expected.  Returned data will be placed into
	the selected FIFO.\\
2'b11 & R1b (48b+BUSY) response expected.  If bits[7:6] are also zero,
	this response type will instead initiate a reset.
	\\\hline
\end{tabular}
%% }}}
...
If set, the ``D'' bit will enable a DMA operation.  If the design has not been
built with any DMA installed ({\tt OPT\_DMA=0}), an error will result instead.
...
responsive to a ``card detect'' input.  Whether or not a card is currently
...
The DMA and BOOT controllers may also set the error bit.
The E$_B$, D$_{\mbox{\tiny E}}$, E$_t$ and E$_c$ bits exist so you can know
which of these types of errors triggered the error bit.
...
and so forth.  Feel free to examine the various included software drivers for
examples of how this may be accomplished.

One item to note before closing: there is only one internal address register
when accessing the FIFO from the Wishbone bus (AXI-Lite) control port.  Attempts
to read from or write to either FIFO from the bus will increment this address 
register.  Interleaved read, or write attempts, such as reading one item from
...
software for security purposes.\footnote{There should be an example of how to
clear the FIFOs found in the example driver software.}
%% }}}
...
LgFSz & 4 & Reflects the {\tt LGFIFO} parameter, and hence the log based two
	of the maximum transfer block size in bytes. \\
...
W & 2 & Width.  Controls how many data bits are used in any data transaction.
	...
	DAT[7:0].  See also Tbl.~\ref{tbl:PHY-WIDTH}.  Defaults to
	{\tt BOOT\_MODE[1:0]} if {\tt OPT\_AUTOBOOT} is set, or
	2'b00 otherwise.\\
D$_S$&1& Data strobe.  Set to one to use the eMMC data strobe, clear
	...
	capability.  Defaults to {\tt BOOT\_MODE[3]} if {\tt OPT\_AUTOBOOT}
	is set, or 1'b0 otherwise.\\
D$_D$ & 1 & DDR.  Set to one to transmit on both edges of the clock.  Clear
	to send data on the positive edge of the clock only.  Defaults to
	{\tt BOOT\_MODE[2]} if {\tt OPT\_AUTOBOOT} is set, or 1'b0 otherwise.\\
CKDIV & 8 & Clock division control.  See Tbl.~\ref{tbl:CKDIV} for more
	information.
	Defaults to {\tt BOOT\_SPEED} if {\tt OPT\_AUTOBOOT} is set,
	{\tt DEF\_SPEED} otherwise.
	\\\hline
\end{tabular}
%% }}}
...
the PHY register will also map illegal user requests to their closest supported
capability.
...
will quietly be downgraded to a 4~bit request.  This feature may be used to
find out how many bits the controller actually supports, as discussed in
Sec.~\ref{sec:phy-query}.
...
8'h1 & 100~MHz clock.  Requires either {\tt OPT\_SERDES} or {\tt OPT\_DDR}.\\
8'h2 &  50~MHz clock\\
8'h3 &  25~MHz clock\\
8'h4 &  12.5~MHz clock\\
%% 8'hfc ->  100kHz (about)
...
either {\tt OPT\_SERDES} or {\tt OPT\_DDR}: set the clock to its highest
speed, and see which speed is permitted, as in Sec.~\ref{sec:phy-query}.
...
the return data.  This requires both {\tt OPT\_EMMC} and {\tt OPT\_DS}.  This
mode may be enabled in the PHY register.  Doing so will force all data bits
returned by the external device to be clocked using this return data strobe.
A separate control bit, $D_C$ enables ``enhanced data strobe support'' as
well.  If set, command reply bits will also be clocked by the return strobe.
...
reset request described above, this reset will be forwarded to the external
device via the hardware reset pin.  The specifc soft reset sequence has been
carefully designed so as to generate a hard reset if the hard reset pin
is available.

While SDIO devices don't typically have hard reset pins, the {\tt OPT\_HWRESET}
pin may still be used to power off the SDIO device--such as when it is
necessary to change voltages back to 3.3~V.
%% }}}
\section{DMA Usage}
%% {{{
...
DMA operations during BOOT mode will be terminated by either raising the CMD
line (normal BOOT mode), or by sending a CMD0 ({\tt GO-IDLE}) command with
a zero argument.  Otherwise, all DMA operations will be terminated with a
CMD12 ({\tt STOP\_TRANSMISSION}).

%% }}}
\section{PHY TRIM}
%% {{{
When sampling via the DS source clock, it may be a challenge to get timing
done properly.  For this reason, the PHY TRIM register exists--to allow
the timing associated with each.  As shown in Fig.~\ref{fig:TRIM},
\begin{figure}\begin{center}
%% {{{
\begin{bytefield}[endianness=big,bitwidth=0.03\linewidth]{32}
\bitheader{0-31}\\
...
\end{bytefield}
%% }}}
\caption{PHY Front end configuration register}\label{fig:TRIM}
\end{center}\end{figure}
and Tbl.~\ref{tbl:PHY},
\begin{table}\begin{center}
%% {{{
\begin{tabular}{|p{1.2in}|p{0.5in}|p{3.5in}|}\hline
	\rowcolor[gray]{0.85} Name & Width & Description\\\hline\hline
Dly[$x$] & 4 & Controls a per--IO bit sub--clock delay, for the purpose of
	adjusting sampling timing.\\\hline
\end{tabular}
%% }}}
\caption{PHY Front end configuration register fields}\label{tbl:TRIM}
\end{center}\end{table}
each IO bit is allowed a delay from 0--15.

In the case of a Xilinx FPGA, the delay values are used to control the bottom
5~bits of an {\tt IDELAYE2} module.

The default (nominal) value of 0 should be good enough for most purposes.

One difficulty experienced during testing the HS200 tuning command on an eMMC
device, was that it was possible for all tuning delays to pass testing.
The example software, therefore, uses nominal delays of 2 for all HS200
mode values.  During tuning, these delays are pushed to either side to help
guarantee tuning failures on the edges of the eye.
%% }}}

\section{RX TRIM}
%% {{{
The receive trim register is designed to allow a configuration to exist which
will shut down the clock prior to all data being received.  The goal of this
approach is to help stop the clock on a dime once all data has been requested,
and once the rest of any incoming data remains in the pipeline.
Apart from this capability, it is possible for an external device to begin
sending a next block of data as part of a multi--block read operation before
the controller is ready for it.  This is a particular issue at higher speeds,
such as HS200 or HS400.  If this happens, the controller will likely miss the
start bit of that second block and (assuming it's not all ones) then fail its
CRC.  To prevent this from happening, the clock must stop long enough between
blocks to allow the controller to reset itself prior to receiving the next
block.  This control helps to make sure the clock stops early enough at
higher speeds.

The PHY TRIM register controls the incoming delays across all of the data pins
associated with the design.  The RX TRIM register finishes the optional
delays, by allowing for a user controlled pre-sample delay for both the CMD
and DS pins.
...
\end{bytefield}
%% }}}
\caption{PHY Front end configuration register}\label{fig:RXTRIM}
\end{center}\end{figure}
and Tbl.~\ref{tbl:PHY},
\begin{table}\begin{center}
%% {{{
\begin{tabular}{|p{1.2in}|p{0.5in}|p{3.5in}|}\hline
	\rowcolor[gray]{0.85} Name & Width & Description\\\hline\hline
CMD Trim & 4 & Delays the clearing of the asynchronous command FIFO reset
	by this many clocks, to keep the asynchronous FIFO from capturing
	outputs and misreading a start signal.
	\\\hline
Rx Trim & 4 & Once the controller is this many clock cycles prior to receiving
	the last data, a clock shut down will be enabled (assuming it is
	also enabled in the PHY register).\\\hline
DS Dly & 4 & Controls an incoming delay on the DS. \\\hline
CMD Dly & 4 & Controls an incoming, pre--sample delay on the CMD wire.
	\\\hline
\end{tabular}
%% }}}
\caption{PHY Front end configuration register fields}\label{tbl:RXTRIM}
\end{center}\end{table}
show the allocation of the bits for both the RX Trim and the two remaining
incoming pin delay controls.

The command pin's delay should nominally be identical to the IO pin delays
from the TRIM register.

The DS pin's delay is different.  This should nominally be set to achieve
a 90~degree phase shift in the clock--so that the DS clock would sample
incoming data in the center of the eye.  In most cases, this should be
an additional 4~units of delay beyond those for the IOs and the CMD wire.

Turning to the RX Trim field of this register, with zero RX Trim, the receive
clock will stop once the last incoming clock's worth of data is registered.
This includes both data and CRC.  Hence, in 1b mode, a 512 Byte sector will
require 4112~clocks containing return data, whereas in 8b mode the same sector
would require 528~clocks.  Once this number, less the RX Trim, has been
received, a flag will be set to allow the clock to stop.  From that point on,
no more clocks will be generated for this receive block.

In the case of HS400, the number of 100~MHz system clock cycles of return
data per block would be 132 (at a 200~MHz clock).  Adjusting the receive trim
register will shut the clock down before this number has been received, by
however much the trim value is set to.

The ``correct'' value for the RX Trim is a matter of active research.  If this
delay is too large, the controller will likely hang on every block.  (A soft
reset should be able to resolve this, to allow proper testing.)  If this delay
is too small, it is possible for the external device to start sending a
subsequent block in a multi--block transfer prior to the controller being
ready.  It is quite likely that this ``correct'' value will be clock rate
dependent.

When working with Xilinx Series-7 devices, an RX Trim delay of 9 was found
to be acceptable for HS200, while an RX Trim of 8 was found acceptable in
HS400.  Note that in both HS200 and HS400 modes, this RX Trim delay is
compared against a count that goes two per clock.  Hence, adjacent pairs
of RX Trim values will have identical performance.

Finally, the CMD Trim setting is only used in HS400 mode when the enhanced
DS is enabled. (D$_C$ in the PHY register.)  Nominally, the asynchronous
command FIFO will be released from reset as soon as the last CMD bits are
sent to the PHY for transmission.  In practice, releasing the asynchronous
FIFOs this early could cause an outgoing CMD bit to be interpreted as a start
bit.  This field then controls how many clock cycles after the last command
bit is sent before releasing the CMD asynchronous FIFO from reset.

When working with Xilinx Series-7 devices, a CMD Trim value of 4 was found
to be acceptable.
%% }}}
...
\item If both {\tt OPT\_CRCTOKEN} and {\tt OPT\_BOOTEN} are set, and a
	BOOT command is given that does not use the DMA, then an interrupt
	will be generated with the ACK or NAK is received from the external
	device.
\item During any BOOT operation that does not use the DMA, an interrupt will
	be generated following the transfer of any block.
\item An interrupt will also be generated at the end of any BOOT operation.
...
\item If the {\tt OPT\_CARD\_DETECT} parameter is set, then an interrupt will
	also be generated any time a card is inserted.  Unlike the other
	interrupts, this interrupt will only clear once the ``Card removed''
	bit is also cleared.
\end{enumerate}
...
The particular constraint on the clock, shown in Chapt.~\ref{ch:clk}, is not
really a Wishbone (or AXI--Lite) constraint, but rather an SD--Card constraint.
Not all cards can handle clocks faster than 25~MHz.  For this reason, the
Wishbone clock, which forms the master clock for this entire controller, must
be divided down so that the external SDIO device clock is within the limits
the card can handle.
...
and controlled by the {\tt H} bit of the CMD register.  This bit may also
be used by SDIO devices as a reset control wire if desired.
