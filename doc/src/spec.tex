...
Setting the {\tt OPT\_SPI\_ARBITRATION} parameter to zero will remove this
arbitration logic for those designs that don't need it.
...
Individual commands may or may not use the FIFO.  Commands that need
...
This core supports two separate FIFO data sections.  This allows a program to
...
been removed and so notify the driver.  This interface was added to simplify
...
{\tt \#define} & {\tt SD\_CRCERR}   & {\tt 0x10000} \\
{\tt \#define} & {\tt SD\_ERRTOK}   & {\tt 0x20000}
\end{tabular}
...
card detect input, adjusted for negative logic.  If this bit is clear (normal
operation), then a card is present and ready to be set up.  On the other hand,
if the card detect line ever goes low then the second bit, {\tt SD\_REMOVED},
will be set.  This bit is sticky.  Once set, it will remain set until explicitly
written to.  This allows us to clear it on an {\tt SD\_GO\_IDLE} command, and
...
\bitbox{1}{T}	% TOKERR,  0x20000
\bitbox{1}{C}	% CRCERR,  0x10000
...
timed out, or a CRC error was noted while reading from the card.  On an
incoming CRC read error, the $C$RCERR bit will be set.  To detect a CRC error
when writing to the card, check the R1 response according to the
SD Card specification.  The $T$ bit indicates a token error.
Errors may be cleared by writing a 1 to the $E$ bit.
Error conditions will persist until cleared.
