...
`endif
	...
	always @(*)
	begin
		if (!startup_hold && (!i_cs || (OPT_SPI_ARBITRATION &&
				(!i_bus_grant
	...
	begin
		...
		begin
			...
			if (o_cs_n)
				...
				|| (i_bus_grant && r_state != LLSDSPI_WAIT)))
		begin
	...
	//	&&( (i_cs)&&(!OPT_SPI_ARBITRATION || i_bus_grant) );
...
`endif
	...
	if (r_state == LLSDSPI_WAIT)
		...
		assert(o_cs_n || i_bus_grant);

	always @(posedge i_clk)
	if (f_past_valid && $past(i_cs && !o_cs_n && !i_bus_grant))
		assert(!o_cs_n && o_sclk);
